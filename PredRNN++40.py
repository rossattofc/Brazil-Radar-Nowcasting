...
def train_model(start_epoch=0):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        np.save(r'path/to/error/file'%(epoch), np.array(loss_list))
        np.save(r'path/to/loss/file'%(epoch), np.array(loss_test_list))
        torch.save(model.state_dict(), r'path/to/model/save' % (epoch))
...
def draw_prediction():
    ...
    np.save(os.path.join(path,'target20'),np.array(y.detach().cpu()))
    np.save(os.path.join(path,'input20'),np.array(x.detach().cpu()))
    np.save(os.path.join(path,'prediction20'),np.array(y_pred))
    
    
    fig, axes = plt.subplots(nrows=3, ncols=20, figsize=(36, 24), sharex=True, sharey=True)
    
    
    for i in range(20):
        axes[0][i].imshow(x[0, i, :, :],cmap="gray")
        axes[1][i].imshow(y[0, i, :, :],cmap="gray")
        axes[2][i].imshow(y_pred[0, i+19, :, :, 0],cmap="gray")  
    plt.tight_layout() 
...
if __name__ == '__main__':
    ...
    data = np.load(r'path/to/dataset')

    data = data / 255
    # draw_sequence(data[:, 0, :, :])

    train_len = 400
    X_train, X_test = data[:20, :400, :, :], data[:20, 100:, :, :]
    Y_train, Y_test = data[20:, :400, :, :], data[20:, 100:, :, :]
    ...
    mask_true = torch.zeros([2, 38, 16, 16, 16])
    mask_true[:, :19, :, :, :] = 1
    mask_true = mask_true.to(device)
    #if torch.cuda.is_available():
        #model.load_state_dict(torch.load('PredRNN/model-saved.pt'))
    #else:
        #state_dict = torch.load('PredRNN/model-saved.pt',
                                #map_location=torch.device('cpu'))
        #model.load_state_dict(state_dict)
        
    #draw_prediction()
    #train_model(start_epoch=0)
    #To train or generate predictions, simply uncomment the specific lines for each function.