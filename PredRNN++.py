...
def draw_prediction():
    ...
    np.save(os.path.join(path,'input'),np.array(x.detach().cpu()))
...
if __name__ == '__main__':
    ...
    mask_true = torch.zeros([2, 18, 16, 16, 16])
    mask_true[:, :9, :, :, :] = 1
    mask_true = mask_true.to(device)
    #if torch.cuda.is_available():
        #model.load_state_dict(torch.load('PredRNN/epochs/model-saved.pt'))
    #else:
        #state_dict = torch.load('PredRNN/epochs/model-saved.pt',
